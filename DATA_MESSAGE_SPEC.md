...
### 이번 주 제출용 샘플 (GitHub 포함)

| 항목 | 값 |
|---|---|
| 방식 | 위 대용량 데이터셋 중 한 파트에서 완전한 세션 단위로 무작위 선택 |
| Kafka 샘플 | **1,910건** (권장 1,000건 이상 충족) |
| 샘플 내 다양성 | 유저 47명, 영화 45편 |

## 데이터 규모 관련 결정 사항

이번 주 실제 제출물(Kafka 전송, GitHub 업로드)은 과제 요구사항(권장 1,000건, 대용량 원본 제외)에 맞춰 작게 유지했습니다. 다만 그 기반이 되는 이벤트 데이터셋 자체는 ml-25m 원본 2,500만 건 중 42만 건(부트스트랩 없이 서로 다른 행)을 사용해 **2,848만 건 규모로 대용량 생성**했습니다. 이렇게 나눈 이유는, 대용량 데이터셋은 다음 단계(부하 테스트·장애 실험)에서 계속 활용하고, 이번 주 채점 대상인 Kafka/GitHub 제출물은 과제 요구사항에 맞는 작은 샘플로 분리하기 위함입니다.
