...
**왜 이 문제를 골랐는지**: 대부분의 데이터 파이프라인 프로젝트는 "정상 동작"까지만 만들고 끝나는데, 실제 현업에서는 "장애가 났을 때 어떻게 되는가"가 더 중요한 경우가 많다고 생각하기 때문이다. 이번 프로젝트는 정상 케이스뿐 아니라 장애 케이스까지 직접 재현하고 대응 과정을 기록하는 것을 목표로 한다.
...
| 버전 | 규모 | 사용 시점 | 용도 |
|---|---|---|---|
| ml-latest-small | 평점 약 10만 건, 영화 약 9,700편 | 2~5회차 (개발 단계) | 빠른 개발/디버깅 사이클 |
| ml-25m | 평점 약 2,500만 건, 영화 약 62,000편 | 6회차 (로드 테스트/장애 대응) | 지속적인 부하를 만들어내는 소스 데이터 |
...
MovieLens는 "유저가 영화에 평점을 남겼다"는 정적 스냅샷이지, "재생했다가 스킵했다" 같은 세분화된 시청 행동 로그가 아니다. 따라서 평점 타임스탬프를 기준점으로 삼아서, 그 전후로 아래와 같은 이벤트를 확률분포 기반으로 합성한다.
...
- 파티션 수: 초기값으로 시작, 6회차 로드 테스트 결과 보고 조정
...
- watermark: late data(네트워크 지연 등으로 늦게 도착하는 이벤트) 처리를 위해 설정, 구체적인 지연 허용 범위는 2회차에서 실험 후 확정
...
- 세션 타임아웃 기준(30분)과 watermark 지연 허용 범위 → 2회차에서 실험 후 확정
- Kafka 파티션 수 → 6회차 로드 테스트 결과를 보고 조정
...
└── raw/
    ...
    │   ├── links.csv
    │   └── ratings.csv
    └── ml-25m/               # 레포에 미포함 (용량 문제, 6회차 로드 테스트용)
```
