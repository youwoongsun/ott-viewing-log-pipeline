...
### 11-3. 저장소 정리 — 계획 vs 완료 표시 정리

이번에 코드에 없는데 있는 것처럼 적혀 있던 부분을 정리했다:

| 항목 | 이전 표기 | 수정 |
|---|---|---|
| TMDB API (2-2) | 실제 사용 중처럼 서술 | "계획 — 아직 실제 연동 안 됨"으로 표시 |
| 이벤트 종류 (2-3) | `play/pause/seek/complete/drop` | 실제 생성기가 만드는 `session_start/segment_watch/session_end`로 정정 |
| Spark 세션화 방식 (3-2) | `mapGroupsWithState`(최초 계획) | 실제 구현인 `session_window`로 정정 (이전 라운드에서 이미 수정) |
| 저장 스키마 (3-3) | "예상 스키마 (확정 아님)" | 실제 확정 스키마 표로 교체 (이전 라운드에서 이미 수정) |
| 6장 장애 실험 상태 | "진행 중 (제출 범위 밖)" | "완료됨 — 8장 참고"로 정정 (이전 라운드에서 이미 수정) |

남아있는 "계획" 항목(7장 정규화 계층/스키마 레지스트리, 8-7의 미해결
이슈들)은 실제로 구현 전이므로 그대로 "계획"/"남은 작업"으로 유지한다.
...
**구성도 파일 자체(`ott_pipeline_architecture.drawio`)는 이 텍스트
다이어그램 기준으로 별도 갱신이 필요하다** — 저장소 전체를 스캔해서
자동으로 그려주는 [archify](https://github.com/tt-a1i/archify) 같은 도구를
써도 되고, 위 텍스트 구조를 그대로 draw.io에 옮겨도 된다.

## 12. 발표 자료 개요

아래 6개 항목과 이 README의 대응 위치:

1. **문제와 데이터** → 1장, 2장
2. **파이프라인 구조와 데이터 모델** → 3장, 11-4 최신 구성도
3. **실행 결과(입력·단계별·최종 저장 건수 표)** → 8-3-1 기준/대용량 비교표
4. **부하·장애·복구에서 확인한 것과 아직 보장 못하는 것** → 8-6 장애 실험
   요약(확인한 것) + 8-7 남은 작업(못 보장하는 것)
5. **저장 결과를 쓰는 장면** → 11-1 (FastAPI `/stats`, `/sessions` 응답)
6. **남은 문제와 다음 단계** → 8-7, 7장

발표 중 라이브로 돌릴 부분은 **11-2의 Airflow 단일 트리거 하나만**
(1~2분 내 종료, 실패해도 같은 명령으로 재실행 가능). 대용량 실행
(500만 건)과 장애 재현(A~E)은 8장의 캡처로 대체하고 실시간으로 돌리지
않는다.
